// Lucide Icon Helper - Client-side SVG icon generation
// Matches the backend lucide_icons.py icon system

// Lucide icon SVG path data - subset for client-side use
const LUCIDE_PATHS = {
    // Core UI icons
    ...
    'trophy': '<path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22H17c0-1.76-.85-3.25-2.03-3.79-.5-.23-.97-.66-.97-1.21v-2.34z"/><path d="M18 2H6v7a6 6 0 0 0 12 0z"/>',
    ...
    // Event choice icons (used in general events)
...
/**
 * Generate a Lucide SVG icon
 * @param {string} iconName - The name of the Lucide icon
 * @param {number} width - SVG width (default: 24)
 * @param {number} height - SVG height (default: 24)
 * @param {string} color - Stroke color (default: 'currentColor')
 * @param {string} cssClass - CSS class for styling (default: 'icon')
 * @returns {string} SVG markup string
 */
function generateLucideSVG(iconName, width = 24, height = 24, color = 'currentColor', cssClass = 'icon') {
    const pathData = LUCIDE_PATHS[iconName] || LUCIDE_PATHS['help-circle'];

    return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-${iconName} ${cssClass}">${pathData}</svg>`;
}

/**
 * Get Lucide SVG for an emoji
 * @param {string} emoji - The emoji character
 * @param {object} options - SVG options (width, height, color, cssClass)
 * @returns {string} SVG markup string
 */
function getIconForEmoji(emoji, options = {}) {
    const iconName = EMOJI_TO_LUCIDE[emoji] || 'help-circle';
    return generateLucideSVG(
        iconName,
        options.width || 24,
        options.height || 24,
        options.color || 'currentColor',
        options.cssClass || 'icon'
    );
}
